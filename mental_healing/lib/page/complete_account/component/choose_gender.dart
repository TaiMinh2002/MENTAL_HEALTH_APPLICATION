...
class _ChooseGenderState extends State<ChooseGender> {
  ...
  Widget _chooseWidget({
    ...
    return Obx(() {
      bool selected = controller.gender.value == value;

      return GestureDetector(
        onTap: () {
          setState(() {
            if (controller.gender.value == value) {
              controller.setGender(0); // Không chọn gì
            } else {
              controller.setGender(value); // Lưu giới tính được chọn
            }
          });
        },
        child: Padding(
